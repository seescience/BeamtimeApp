 ...
 */
export function performClientSideSearch(searchTerm: string): void {
    const tableBody = document.getElementById('experimentsTableBody');
    if (!tableBody) return;
    
    const rows = tableBody.querySelectorAll<HTMLTableRowElement>('.experiment-row');
 ...
 */
export function initializeSearchFunctionality(): void {
